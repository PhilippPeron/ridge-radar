...
import { Text, TouchableOpacity, View, Pressable, TextInput } from "react-native";
...
export default function Modal() {
    ...
    if (newLoc) {
        ...
        locData = globalLocations.locations.find((loc) => loc.id === locId) as Location;
        if (!locData) {
...
function updateLocation(locData: Location) {
    ...
    const existingLoc = globalLocations.locations.find((loc) => loc.id === locData.id);
    if (existingLoc) {
...
function deleteLocation(locId: number) {
    ...
    globalLocations.locations = globalLocations.locations.filter((loc) => loc.id !== locId);
    dataStorer.saveToStorage("locations", globalLocations);
